        ...
        public IEnumerable<string> GetQueues()
        {
            lock (_cacheLock)
            {
                if (_queuesCache.Count == 0 || _cacheUpdated.Add(QueuesCacheTimeout) < DateTime.UtcNow)
                {
                    ...
                    {
                        return connection.Query("select distinct(Queue) from HangfireJobQueue").Select(x => (string)x.Queue).ToList();
                    });
...
where r.rank between @start and @end;";
        ...
        public EnqueuedAndFetchedCountDto GetEnqueuedAndFetchedCount(string queue)
        {
            return _storage.UseConnection(connection =>
            {
                var result = 
                    connection.Query<int>(
                        "select count(Id) from HangfireJobQueue where Queue = @queue", new { queue = queue }).Single();
